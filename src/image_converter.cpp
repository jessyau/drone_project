	...
	if (mouseUp == true) {
		...
		if (searchImg == true) {
			...
			if (lost == true) {
				...
				int type1 = cv_ptr->image.type();
				int type2 = croppedImage.type();
				cout << "Search for image, first time!" << endl;
				
				cout << "Camera type: " << type1 << "\tcroppedImage type: " << type2 << endl;
				if (type1 != type2) {
					//convert from CV_8U to CV_8UC3
					croppedImage.convertTo(croppedImage, CV_8UC3, 1.0/255.0);
				}
				type1 = cv_ptr->image.type();
				type2 = croppedImage.type();
				cout << "2nd attempt camera type: " << type1 << "\tcroppedImage type: " << type2 << endl;
